...
minetest.register_node("lib_ecology:tree_generic_trunk_jungle_03", {
	description = S("Jungle Tree 03 Trunk"),
	tiles = {
		"lib_ecology_tree_generic_trunk_top_jungle_03.png",
		"lib_ecology_tree_generic_trunk_top_jungle_03.png",
		"lib_ecology_tree_generic_trunk_jungle_03.png"
	},
	paramtype = "light",
	paramtype2 = "facedir",
	walkable = true,
...
minetest.register_node("lib_ecology:jungle_trunk_with_red_leaves", {
	description = S("Jungle Tree Trunk with Red Leaves"),
	tiles = {
		"lib_ecology_tree_jungle_trunk_top.png",
		"lib_ecology_tree_jungle_trunk_top.png",
		"lib_ecology_tree_jungle_trunk.png^lib_ecology_tree_leaves_jungle_red.png"
	},
	paramtype2 = "facedir",
...
minetest.register_node("lib_ecology:tree_trunk_with_leaves", {
	description = S("Tree Trunk with Leaves"),
	tiles = {
		"default_tree_top.png",
		"default_tree_top.png",
		"default_tree.png^default_leaves.png"
	},
	paramtype2 = "facedir",
	is_ground_content = true,
	groups = {tree=1,snappy=1,choppy=2,oddly_breakable_by_hand=1,flammable=2},
...
minetest.register_node("lib_ecology:savannah_trunk_with_jungle_leaves", {
	description = S("Savannah Tree Trunk with Jungle Leaves"),
	tiles = {
		"mg_dry_tree_top.png",
		"mg_dry_tree_top.png",
		"mg_dry_tree.png^default_jungleleaves.png"
	},
	paramtype2 = "facedir",
	is_ground_content = true,
	groups = {tree=1,snappy=1,choppy=2,oddly_breakable_by_hand=1,flammable=2},
...
minetest.register_craft({
	...
	recipe = {
		{'lib_ecology:tree_generic_trunk_tree_02'},
	}
})
minetest.register_craft({
	output = 'default:wood 4',
	recipe = {
		{'lib_ecology:tree_generic_trunk_tree_03'},
	}
})
...
dofile(lib_ecology.path .. "/bush.lua")
--dofile(lib_ecology.path .. "/schematics/waterlily.lua")
--dofile(lib_ecology.path.."/schematics/bigtree.lua")
--dofile(lib_ecology.path.."/schematics/vinetree.lua")
--dofile(lib_ecology.path.."/schematics/palmtree.lua")
--dofile(lib_ecology.path.."/schematics/redwood_tree.lua")
--dofile(lib_ecology.path.."/schematics/frosttrees.lua")
--dofile(lib_ecology.path.."/schematics/yellowtree.lua")


-- Valleys_c tree schematics
dofile(lib_ecology.path.."/lib_ecology_tree_banana.lua")
--dofile(lib_ecology.path.."/lib_ecology_tree_conifer.lua")
dofile(lib_ecology.path.."/lib_ecology_tree_deciduous.lua")
--dofile(lib_ecology.path.."/lib_ecology_tree_jungle.lua")
--dofile(lib_ecology.path.."/lib_ecology_tree_pine.lua")

--Fern Trees
dofile(lib_ecology.path.."/lib_ecology_tree_fern.lua")

--Trees
--dofile(lib_ecology.path.."/lib_ecology_tree_conifer2.lua")
--dofile(lib_ecology.path.."/lib_ecology_tree_jungle2.lua")
--dofile(lib_ecology.path.."/lib_ecology_tree_mangrove.lua")
--dofile(lib_ecology.path.."/lib_ecology_tree_palm.lua")
--dofile(lib_ecology.path.."/lib_ecology_tree_date_palm.lua")

--Conifers tree schematics
--dofile(lib_ecology.path.."/lib_ecology_tree_conifer3.lua")


--THESE MUST BE RAN AFTER ABOVE CODE
dofile(lib_ecology.path.."/lib_ecology_ltree_models.lua")
dofile(lib_ecology.path.."/lib_ecology_schem_models_1.lua")
--dofile(lib_ecology.path.."/lib_ecology_schem_models_2.lua")
...
--[[
	minetest.register_on_generated(function(minp, maxp, blockseed)
		...
		--apple tree
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_dark_with_grass3", })
		if pos ~= nil then
			minetest.spawn_tree({x=pos.x, y=pos.y+1, z=pos.z}, lib_ecology.apple_tree_model)
		end
		
		--cedar tree
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_coarse_with_dry_grass3", "lib_materials:dirt_clay_red_with_dry_grass4", "lib_materials:dirt_clayey_with_dry_grass4}", })
		if pos ~= nil then
			minetest.spawn_tree({x=pos.x, y=pos.y+1, z=pos.z}, lib_ecology.cedar_model)
		end
		
		--conifer
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_silty_with_snow", })
		if pos ~= nil then
			lib_ecology.grow_conifertree({x = pos.x, y = pos.y, z = pos.z})
		end
		
		--date palm
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_coarse_with_dry_grass4", "lib_materials:dirt_clay_red_with_dry_grass5"})
		if pos ~= nil then
			minetest.spawn_tree({x=pos.x, y=pos.y+1, z=pos.z}, lib_ecology.date_palm_model)
		end
		
		--fir tree
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_dark_with_snow", })
		if pos ~= nil then
			minetest.spawn_tree({x=pos.x, y=pos.y+1, z=pos.z}, lib_ecology.fir_model)
		end
		
		--jungle tree -valleys_c-
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_dark_with_brown_grass4", })
		if pos ~= nil then
			lib_ecology.grow_jungletree(pos)
		end
		
		--mangrove tree
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_silt_02", })
		if pos ~= nil then
			lib_ecology.grow_mangrovetree({x = pos.x, y = pos.y, z = pos.z})
		end
		
		--oak tree
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_dark_with_dry_grass3", })
		if pos ~= nil then
			minetest.spawn_tree({x=pos.x, y=pos.y+1, z=pos.z}, lib_ecology.oak_model)
		end
		
		--palm tree
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:stone_sandstone_white_gravel", })
		if pos ~= nil then
			minetest.spawn_tree({x=pos.x, y=pos.y+1, z=pos.z}, lib_ecology.palm_model)
		end
		
		--palm 2 tree
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:stone_sandstone_white_gravel", })
		if pos ~= nil then
			minetest.spawn_tree({x=pos.x-6, y=pos.y+1, z=pos.z+6}, lib_ecology.palm2_model)
		end
		
		--poplar tree small
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_dark_with_grass2", })
		if pos ~= nil then
			minetest.spawn_tree({x=pos.x, y=pos.y+1, z=pos.z}, lib_ecology.poplar_small_model)
		end
		
		--rubber tree
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_with_rainforest_litter", })
		if pos ~= nil then
			minetest.spawn_tree({x=pos.x, y=pos.y+1, z=pos.z}, lib_ecology.rubber_tree_model)
		end
		
		--fern tree giant
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_silty_with_rainforest_litter", })
		if pos ~= nil then
			lib_ecology.grow_giant_tree_fern({x = pos.x, y = pos.y-1, z = pos.z})
		end
		
		--fern tree
		pos = minetest.find_node_near(tmp, maxp.x - minp.x, {"lib_materials:dirt_dark_with_rainforest_litter", })
		if pos ~= nil then
...
minetest.register_abm({
	...
	action = function(pos, node)
		...
		if node.name == "lib_ecology:tree_acacia_sapling" then
			minetest.spawn_tree(pos, lib_ecology.acacia_model)
		end
		
		if node.name == "lib_ecology:tree_apple_sapling" then
			minetest.spawn_tree(pos, lib_ecology.apple_tree_model)
		end
		
		if node.name == "lib_ecology:tree_birch_01_sapling" then
			rand_schem = math.random(1,2)
			if rand_schem == 1 then
				minetest.spawn_tree(pos, lib_ecology.birch_model1)
			elseif rand_schem == 2 then
				minetest.spawn_tree(pos, lib_ecology.birch_model2)
			end
		end
			
		if node.name == "lib_ecology:tree_cedar_sapling" then
			minetest.spawn_tree(pos, lib_ecology.cedar_model)
		end
			
		if node.name == "lib_ecology:tree_conifers_sapling" then
			rand_schem = math.random(1,3)
			if rand_schem == 1 then
				lib_ecology.grow_new_conifer_tree(pos)
			elseif rand_schem == 2 then
				lib_ecology.grow_conifertree(pos)
			elseif rand_schem == 3 then
   				lib_ecology.make_conifer(pos, math.random(0, 1))
			end
		end
			
		if node.name == "lib_ecology:tree_date_palm_sapling" then
			rand_schem = math.random(1,2)
			if rand_schem == 1 then
				minetest.spawn_tree(pos, lib_ecology.date_palm_model)
			elseif rand_schem == 2 then
				lib_ecology.grow_date_palm(pos)
			end
		end
			
		if node.name == "lib_ecology:tree_fir_sapling" then
			minetest.spawn_tree(pos, lib_ecology.fir_model)
		end
					
		if node.name == "lib_ecology:tree_jungle_01_sapling" then
			local rand_schem = math.random(1,3)
			if rand_schem == 1 then
				lib_ecology.grow_jungletree(pos)
			elseif rand_schem == 2 then
				lib_ecology.grow_new_jungle_tree(pos)
			elseif rand_schem == 3 then
				minetest.spawn_tree(pos, lib_ecology.jungletree_model)
			end
		end
			
		if node.name == "lib_ecology:tree_mangrove_sapling" then
			lib_ecology.grow_mangrovetree(pos)
		end
			
		if node.name == "lib_ecology:tree_oak_sapling" then
			minetest.spawn_tree(pos, lib_ecology.oak_model)
		end
			
		if node.name == "lib_ecology:tree_palm_01_sapling" then
			local rand_schem = math.random(1,3)
			if rand_schem == 1 then
				minetest.spawn_tree(pos, lib_ecology.palm_model)
			elseif rand_schem == 2 then
				minetest.spawn_tree(pos, lib_ecology.palm2_model)
			elseif rand_schem == 3 then
				lib_ecology.grow_palmtree(pos)
			end
		end
			
		if node.name == "lib_ecology:tree_pine_02_sapling" then
			minetest.spawn_tree(pos, lib_ecology.pine_model)
		end
			
		if node.name == "lib_ecology:tree_poplar_small_sapling" then
			minetest.spawn_tree(pos, lib_ecology.poplar_small_model)
		end
			
		if node.name == "lib_ecology:tree_rubber_sapling" then
			minetest.spawn_tree(pos, lib_ecology.rubber_tree_model)
		
		end

		if node.name == "lib_ecology:tree_spruce_sapling" then
			local rand_schem = math.random(1,2)
			if rand_schem == 1 then
				minetest.spawn_tree(pos, lib_ecology.spruce_model1)
			elseif rand_schem == 2 then
				minetest.spawn_tree(pos, lib_ecology.spruce_model2)
			end
		end
